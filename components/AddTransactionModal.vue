<template>
  <!-- 1. Gunakan Transition bawaan Vue untuk menangani animasi masuk & keluar -->
  <Transition name="sheet">
    <div v-if="isOpen">
      <!-- Overlay Back-drop -->
      <div class="fixed inset-0 bg-on-surface/40 backdrop-blur-[2px] z-50 transition-opacity" @click="$emit('close')">
      </div>

      <!-- Bottom Sheet Modal -->
      <div id="bottomSheet"
        class="fixed bottom-0 left-0 right-0 z-[60] bg-surface-container-lowest rounded-t-[32px] shadow-2xl flex flex-col max-h-[92vh]">
        <!-- Handle Bar -->
        ...
        <div class="px-container-padding overflow-y-auto custom-scrollbar pb-safe">
          ...
          <div class="bg-surface-container-low rounded-full p-1.5 flex mb-8">
            <button type="button" @click="transactionType = 'expense'" :class="[
            ...
            <button type="button" @click="transactionType = 'income'" :class="[
              'flex-1 py-2.5 rounded-full text-body-md font-semibold transition-all duration-300',
          ...
          <div class="space-y-4 mb-8">
            <!-- Input Tanggal -->
            <div
              class="bg-surface-container-low rounded-2xl p-4 flex items-center gap-4 transition-all focus-within:ring-2 focus-within:ring-primary/20">
            ...
            <!-- Input Catatan -->
            <div
              class="bg-surface-container-low rounded-2xl p-4 flex items-start gap-4 transition-all focus-within:ring-2 focus-within:ring-primary/20">
              ...
              <div class="flex-1">
                ...
                <textarea rows="2" v-model="notes" placeholder="Tulis catatan di sini..."
                  class="w-full bg-transparent border-none p-0 text-body-md text-on-surface focus:ring-0 resize-none placeholder:text-outline-variant"></textarea>
              </div>
            </div>
          </div>
...
<style scoped>
/* 2. Animasi Durasi & Easing */
.sheet-enter-active,
.sheet-leave-active {
  transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1);
}

/* Animasi untuk Overlay Back-drop */
.sheet-enter-active .fixed.inset-0,
.sheet-leave-active .fixed.inset-0 {
  transition: opacity 0.3s ease;
}

.sheet-enter-from .fixed.inset-0,
.sheet-leave-to .fixed.inset-0 {
  opacity: 0;
}

/* Animasi Slide Up & Down untuk Bottom Sheet Modal */
.sheet-enter-active #bottomSheet,
.sheet-leave-active #bottomSheet {
  transition: transform 0.3s cubic-bezier(0.16, 1, 0.3, 1);
}

.sheet-enter-from #bottomSheet,
.sheet-leave-to #bottomSheet {
